...
namespace NerualNet.Logic
{
    public class Net
    {
        ...
        public Func<float[], float[], float[], float[], float> GetTrainingFunction()
        {
            ...
            builder.AppendLine("((float[] inputs, float[] outputs, float[] weights, float[] d) => {");
        ...
        public Func<float[], float[]> GetEvaluationFunction()
        {
            ...
            builder.Append(string.Join(",", _outputNodes.Select(id => $" (float) out{id}")));
            builder.AppendLine("};");
            builder.AppendLine("})");
